...
country_parameters <- read_excel(paste0("LULCC/DownloadedDatasets/SourceData",country_name,"/parameters.xlsx")) 
...
for (lucinputdataset in lucavailablemaps) {
  ...
  if (lucinputdataset == "modis") {
    ...
    luc_2010_df <- luc_2010_dfv0 %>%
      dplyr::mutate(chr_code = as.character(continent)) %>% #CONTINENT WARNING
    ...
    region_f <- data.frame(reg_code = sort(unique(regions_adm0_p_df$continent)), #CONTINENT WARNING
                           reg_chr = c("SSA", "LATAM", "ASIA", "NorAfr")) %>%
    ...
    growth_para_v1 <- inner_join(luc_2010_df, region_f, by = "reg_code") %>%
      ...
      dplyr::rename(IDorig = continent) %>% #CONTINENT WARNING
    ...
    rclmat_cop <- luccopernicus_cat_vx %>%
      ...
      unname() %>%
      as.matrix()
    luccopernicus_2015_rcr <- classify(luccopernicus_2015, rclmat_cop, include.lowest=TRUE)
    ...
    luc_2015_df_cop <- luc_2015_dfv0_cop %>%
      dplyr::mutate(chr_code = as.character(lyr2)) %>% # continent WARNING FIX
      separate(chr_code, into = c('reg_code', 'temp'), sep = 1) %>%
      separate(temp, into = c('gez_code', 'luc_code'), sep = 2) %>%
      dplyr::mutate_at(c('reg_code','gez_code','luc_code'), as.numeric)
    ...
    growth_para_v1 <- inner_join(luc_2015_df_cop, region_f, by = "reg_code") %>%
      ...
      tidyr::unite("reg_gez_luc", c(reg_chr, gez_name, luc_cat), sep= "_", remove = FALSE) %>%
      tidyr::unite("reg_gez", c(reg_chr, gez_name), sep= "_", remove = FALSE) %>%
      dplyr::rename(IDorig = lyr2) %>% # continent WARNING FIX
      dplyr::relocate(reg_gez, .after = luc_cat) %>%
      dplyr::relocate(reg_gez_luc, .after = reg_gez) %>%
      dplyr::select(-lyr1) # OJO ACA!
    head(growth_para_v1)
    # Find duplicates
    dup <- growth_para_v1 %>% 
      group_by(IDorig) %>% 
  ...
  agb_mean_decil <- agb_mean_decilv0 %>%
    as.data.frame() %>%
    dplyr::rename(IDorig = lyr1, # Warning
                  agb_mean_Tdecil = agb) %>%
  ...
  agb_sd_decil <- agb_sd_decilv0 %>%
    as.data.frame() %>%
    dplyr::rename(IDorig = var_name1, # Warning
                  agb_sd_Tdecil = agb) %>%
    dplyr::inner_join(growth_para_v1, by = "IDorig") %>%
    dplyr::select(-reg_code:-reg_gez_luc, -TOF_luc) %>%
    replace(., is.na(.), 1) # Replace  NaN for no pixels in category!
  ...
  if (lucinputdataset == "modis") {
    ...
    growth_parameters_v3 <- growth_parameters_v2 %>%
      dplyr::select(c("Key",
                      "reg_gez_luc",
                      "r",
                      "rSD",
