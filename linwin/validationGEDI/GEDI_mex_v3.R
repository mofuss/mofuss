...
if (filtering == "se"){
  GEDI_Mofuss_loss <- idw_v_mex_df %>% 
    ...
    dplyr::rename(probability_mofuss="IDW_C++_fw_v13.x",
                  AGB_delta="IDW_C++_fw_v13.y",
                  AGB_sd="IDW_C++_fw_v13.x.1",
                  AGB_se="IDW_C++_fw_v13.y.1") %>%
    # dplyr::filter(abs(AGB_delta)>=AGB_se) %>%
    transform(., Change = ifelse(abs(AGB_delta)>(AGB_sd*5) & AGB_delta<0, 1, 0)) %>%
    drop_na(probability_mofuss) %>%
    drop_na(AGB_delta)
} else {
  GEDI_Mofuss <- idw_v_mex_df %>% 
    st_join(dAGB_m_df, by="geometry") %>% 
    dplyr::rename(AGB_delta="IDW_C++_fw_v13.y", probability_mofuss="IDW_C++_fw_v13.x") %>% 
    drop_na(probability_mofuss) %>% 
    drop_na(AGB_delta)
