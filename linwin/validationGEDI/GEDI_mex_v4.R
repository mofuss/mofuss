...
if (filtering == "se"){
  ...
  GEDI_Mofuss <- idw_v_mex_df %>% 
    ...
    drop_na(probability_mofuss) %>% 
    drop_na(AGB_delta) %>% 
    filter(AGB_delta>-400 & AGB_delta<60) %>% 
    st_set_crs(3395)
  
  # Create datasets for Gain and Loss ROC curves
  GEDI_Mofuss_gain <- GEDI_Mofuss %>% 
    mutate(Change = case_when(AGB_delta>-400 & AGB_delta<=(10)~ 'no_gain',
  ...
  GEDI_Mofuss_loss <- GEDI_Mofuss %>% 
    mutate(Change= case_when(AGB_delta>-400 & AGB_delta<=(-10) ~ 'loss',
...
AGBD_delta_plot<- ggplot() +  
  geom_sf(data=GEDI_Mofuss, aes(color = AGB_delta), 
          size = 0.00001) +
  ...
  scale_color_viridis(name="AGBD change (tn/ha)", option = "D") +
  annotation_scale() +
...
roclist_v <- list("AGBD Loss, AUC = 0.591" = roc_loss_v,
                  "AGBD Gain, AUC = 0.577" = roc_gain_v,
                  "AGBD Random, AUC = 0.501" = roc_random_v)
...
##NOW FOR W
# read Mexico raster probability layers
idw_v_mex <- rast("GEDI_in/IDW_C++_fw_w13.tif") #Replace v for w
idw_w_mex <- rast("GEDI_in/IDW_C++_fw_w13.tif")
...
# stack_v <- c(idw_v_mex,dAGB_m,dAGB_sd,dAGB_se)
...
if (filtering == "se"){
  ...
  # Create datasets for Gain and Loss ROC curves
  GEDI_Mofuss_gain <- GEDI_Mofuss %>% 
    mutate(Change = case_when(AGB_delta>-400 & AGB_delta<=(10)~ 'no',
  ...
  GEDI_Mofuss_loss <- GEDI_Mofuss %>% 
    mutate(Change= case_when(AGB_delta>-400 & AGB_delta<=(-10) ~ 'yes',
...
roclist_w <- list("AGBD Loss, AUC = 0.637 " = roc_loss_v,
                "AGBD Gain, AUC = 0.630 " = roc_gain_v,
                "AGBD Random, AUC = 0.501 " = roc_random_v)
