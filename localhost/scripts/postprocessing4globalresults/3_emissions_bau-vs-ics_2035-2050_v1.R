...
# 
# #demanddir <- "C:/Users/aghil/Documents/demand"
# demanddir <- "F:/demand"
# 
# #admindir <- "C:/Users/aghil/Documents/admin_regions"
# admindir <- "F:/admin_regions"
# 
# #emissionsdir <- "C:/Users/aghil/Documents/emissions"
# emissionsdir <- "F:/emissions"
# 
# #rTempdir <- "C:/Users/aghil/Documents/rTemp"
# rTempdir <- "F:/rTemp"
...
for (scex in scenario.list) { # Start scex loop ----
  ...
  for (i in adm0_reg$GID_0) { 
    ...
    for (f in fuelschr) {
      ...
      terra::writeRaster(fuelDem_allE,
                         paste0("emissions_temp_", scex, "/",pop_ver,"_", i, "_", f, "_emissionsSum.tif"),
                         filetype = "GTiff", overwrite = TRUE)
    }
  ...
  if (mergecountries == 1) { 
    ...
    for (f in fuelschr) {
      ...
      emissions_list <- list.files(path = paste0("emissions_temp_",scex,"/"),
                                   pattern = paste0("_",f,".*\\.tif$"), full.names = TRUE)
      
      if (length(emissions_list) > 1) { 
        emissions_raster_list <- lapply(emissions_list, rast)
        emissions_users <- do.call("merge", emissions_raster_list)
        terra::writeRaster(emissions_users,
                           paste0("emissions_out_",scex,"/",pop_ver,"_",firstyr,"-",
                                  lastyr,"_",f,"_",scex,"_emissions.tif"),
  ...
  finalemissions_list <- list.files(path = paste0("emissions_out_",scex,"/"),
                                    pattern = paste0(pop_ver,"_.*\\",scex,"_emissions.*\\.tif$"), full.names = TRUE)
  ...
  terra::writeRaster(eCO2e_pNoAdj, paste0("emissions_out_",scex,"/e",firstyr,"-",
                                          lastyr,"_",scex,"_tCO2e_noadj.tif"),
                     filetype = "GTiff", overwrite = T)
...
if (avoidedemissions == 1){ 
  ...
  if (file.exists(paste0(emissionsdir,"/",lastyr,regiontag,"/BaU/emissions_out_BaU/e",firstyr,"-",lastyr,"_BaU_tCO2e_gcs.tif")) == TRUE &
      file.exists(paste0(emissionsdir,"/",lastyr,regiontag,"/ICS/emissions_out_ICS/e",firstyr,"-",lastyr,"_ICS_tCO2e_gcs.tif")) == TRUE) {
    
    BaU20xxa <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/BaU/emissions_out_BaU/e",firstyr,"-",lastyr,"_BaU_tCO2e_gcs.tif"))    
    ICS20xxa <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/ICS/emissions_out_ICS/e",firstyr,"-",lastyr,"_ICS_tCO2e_gcs.tif"))
    AvEm20xx_gcs_tpp <- BaU20xxa - ICS20xxa # tpp stands for tonnes per pixel per period
    terra::writeRaster(AvEm20xx_gcs_tpp, paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_gcs_tpp.tif"),
                       filetype = "GTiff", overwrite = T)
    
    AvEm20xx_gcs_tpyr <- (AvEm20xx_gcs_tpp/simlength) # tpyr stands for tonnes per pixel per year
    terra::writeRaster(AvEm20xx_gcs_tpyr, paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_gcs_tpyr.tif"),
                       filetype = "GTiff", overwrite = T)
    
    BaU20xxb <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/BaU/emissions_out_BaU/e",firstyr,"-",lastyr,"_BaU_tCO2e.tif"))    
    ICS20xxb <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/ICS/emissions_out_ICS/e",firstyr,"-",lastyr,"_ICS_tCO2e.tif"))
    AvEm20xx_wm_tpp <- BaU20xxb - ICS20xxb #tpp stands for tonnes per pixel per period
    terra::writeRaster(AvEm20xx_wm_tpp, paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_tpp.tif"),
                       filetype = "GTiff", overwrite = T) 
    
    BaU20xxc <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/BaU/emissions_out_BaU/e",firstyr,"-",lastyr,"_BaU_tCO2e_thay.tif"))    
    ICS20xxc <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/ICS/emissions_out_ICS/e",firstyr,"-",lastyr,"_ICS_tCO2e_thay.tif"))
    AvEm20xx_wm_thay <- BaU20xxc - ICS20xxc #thay stands for tonnes per hectare per yr
    terra::writeRaster(AvEm20xx_wm_thay, paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_thay.tif"),
                       filetype = "GTiff", overwrite = T)
...
if (zonalstats == 1){
  ...
  AvEm_gcs_tppr <- raster(paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_gcs_tpp.tif"))
  AvEm_gcs_tpyr <- raster(paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_gcs_tpyr.tif"))
  AvEm_wm_tppr <- raster(paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_tpp.tif"))
  AvEm_wm_thayr <- raster(paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_thay.tif"))
  ...
  AvEm_gcs_tppr_sum <- as.data.frame(zonal(AvEm_gcs_tppr, admin_r, 'sum')) %>%
  ...
  AvEm_gcs_tppr_sum
  setwd(emissionsdir)
  write.csv(AvEm_gcs_tppr_sum,paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_gcs_tpp_sum.csv"), row.names=FALSE, quote=FALSE)
  ...
  AvEm_wm_tppr_sum
  write.csv(AvEm_wm_tppr_sum,paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_tpp_sum.csv"), row.names=FALSE, quote=FALSE)
  ...
  AvEm_wm_thayr_sum
  write.csv(AvEm_wm_thayr_sum,paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_thayr_sum.csv"), row.names=FALSE, quote=FALSE)
  
  
  ## raster and tables summaries for cross validation ----
  AvEm_gcs_tpyrt <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_gcs_tpyr.tif"))
  ...
  AvEm_wm_tpprt <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_tpp.tif"))
  ...
  AvEm_wm_thayrt <- rast(paste0(emissionsdir,"/",lastyr,regiontag,"/AE",lastyr,"_wm_thay.tif"))
