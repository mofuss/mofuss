...
#
# v8 FIX (emission factors): combustion CO2 is now included for every fuel EXCEPT
#   those whose CO2 is already counted in the harvest / Dinamica module
#   (fuelwood, charcoal, imp_fuelwood, imp_charcoal). Previously ef_val = CH4 + N2O
#   for ALL fuels, which silently dropped fossil CO2 for Electricity / Gas /
#   Kerosene / Coal (undercounting them ~9-24x).
#   Inclusion is driven by `co2_in_harvest` (TRUE = CO2 already in harvest module ->
#   exclude here). For all other fuels the EF table's CO2 is used AS-IS:
#     - gas/kerosene/electric/coal: fossil CO2, currently non-zero.
#     - pellets/biogas/ethanol: currently CO2=0 in efdb (renewable feedstock), but the
#       value is read from efdb so a future non-renewable assumption flows through
#       automatically instead of being hardcoded away.
#   CH4 + N2O are always counted (incomplete-combustion gases are not offset by the
#   biogenic carbon cycle), so biomass fuels are never treated as emission-free.
...
# v8: Does this fuel's combustion CO2 already get counted in the harvest / Dinamica
# module (i.e. on the AGB side)? If TRUE, we MUST exclude CO2 here to avoid
# double-counting the same biomass carbon. If FALSE, we use whatever CO2 the EF
# table (efdb_all.csv) provides for this fuel.
#
# Rationale by fuel:
#   fuelwood, charcoal, imp_fuelwood, imp_charcoal -> CO2 counted in harvest module -> EXCLUDE here (structural; never changes).
#   gas, kerosene, electric, coal                  -> fossil CO2 -> INCLUDE efdb CO2.
#   pellets, biogas, ethanol                        -> currently CO2=0 in efdb (renewable feedstock assumption),
#                                                      but we INCLUDE the efdb CO2 so that if a future efdb assumes
#                                                      non-renewable feedstock or adds production-CO2, it flows through.
# NOTE: imp_fuelwood / imp_charcoal are folded into the fuelwood / charcoal groups,
# so the group-level flag below covers them.
co2_in_harvest <- c(
  fuelwood = TRUE,
  charcoal = TRUE,
  gas      = FALSE,
  ...
  biogas   = FALSE,
  coal     = FALSE,
  other    = FALSE   # NOTE: 'other' uses efdb CO2 as-is; revisit if it represents harvested biomass.
)
...
for (g in names(groups)) {
  cat("\n=== Group:", g, "===\n")
  # g = "electric"
  # 4.1) Get EF for this group.
  #   CO2 is EXCLUDED only when it is already counted in the harvest module
  #   (fuelwood/charcoal/imp_*). For every other fuel we use the efdb CO2 as-is,
  #   so future efdb changes (e.g. non-renewable pellets/biogas/ethanol) flow through.
  ef_key <- ef_name_map[[g]]
  row_g  <- efdb %>% filter(fuel_std == ef_key)
  if (nrow(row_g) == 0) {
    warning(sprintf("Missing EF for '%s' (%s) in %s; skipping.", g, ef_key, gid0))
    next
  }
  co2_in_harv <- isTRUE(co2_in_harvest[[g]])
  ef_co2 <- as.numeric(row_g$CO2)
  ef_nonco2 <- as.numeric(row_g$CH4 + row_g$N2O)
  ef_val <- if (co2_in_harv) ef_nonco2 else ef_co2 + ef_nonco2
  cat(sprintf("EF for %-12s [CO2 %s]: CO2=%.5f + CH4+N2O=%.5f -> EF=%.5f tCO2e/unit\n",
              ef_key,
              if (co2_in_harv) "excluded: in harvest module" else "included: from efdb",
              ef_co2, ef_nonco2, ef_val))
  ...
  # 4.6) Convert demand to emissions (tCO2e) using ef_val (CO2 from efdb unless counted in harvest module; always +CH4+N2O); charcoal ÷ efchratio
