...
subregionsSSA_v <- "subregionsSSA_v5TNC_congo.csv" # This basically re-cluster countries in new sub regions, such as Miombo Mopane to be run all toguether
...
# ------------------------------------------------------------------------------
# GADM read + harmonize ADM0/ADM1/ADM2 so that:
# - ADM0 is the master list of GID_0
# - ADM1 contains ALL ADM0 countries (fallback: ADM0 polygon as single ADM1)
# - ADM2 contains ALL ADM0 countries (fallback: ADM1 polygons as ADM2; if none, ADM0 as ADM2)
# - No extra GID_0 values beyond ADM0 (e.g. "NA") survive in ADM1/ADM2
# ------------------------------------------------------------------------------
...
# Add ADM0-as-ADM1 for countries missing ADM1
...
adm0_as_adm1 <- gadm_adm0_sel %>%
  ...
  dplyr::mutate(
    GID_1  = paste0(GID_0, ".0_1"),
    NAME_1 = NAME_0
  ) %>%
  dplyr::select(GID_0, NAME_0, GID_1, NAME_1)

gadm_adm1_sel <- dplyr::bind_rows(gadm_adm1_sel, adm0_as_adm1) %>%
  dplyr::filter(GID_0 %in% adm0_countries)
...
# Countries missing ADM2 -> use ADM1 polygons as ADM2
...
adm1_as_adm2 <- gadm_adm1_sel %>%
  ...
  dplyr::mutate(
    GID_2  = paste0(GID_1, ".0_2"),
    NAME_2 = NAME_1
  ) %>%
  dplyr::select(GID_0, NAME_0, GID_1, NAME_1, GID_2, NAME_2)

gadm_adm2_sel <- dplyr::bind_rows(gadm_adm2_sel, adm1_as_adm2) %>%
  dplyr::filter(GID_0 %in% adm0_countries)

# Still missing (rare) -> use ADM0 polygons as ADM2
...
adm0_as_adm2 <- gadm_adm0_sel %>%
  ...
  dplyr::mutate(
    GID_1  = paste0(GID_0, ".0_1"),
    NAME_1 = NAME_0,
    GID_2  = paste0(GID_0, ".0_1.0_2"),
    NAME_2 = NAME_0
  ) %>%
  dplyr::select(GID_0, NAME_0, GID_1, NAME_1, GID_2, NAME_2)

gadm_adm2_sel <- dplyr::bind_rows(gadm_adm2_sel, adm0_as_adm2) %>%
  dplyr::filter(GID_0 %in% adm0_countries)

gadm_adm2_sel_db <- gadm_adm2_sel %>% st_drop_geometry()

# --- Sanity checks ------------------------------------------------------------
setdiff(unique(gadm_adm0_sel$GID_0), unique(gadm_adm1_sel$GID_0))  # should be character(0)
setdiff(unique(gadm_adm0_sel$GID_0), unique(gadm_adm2_sel$GID_0))  # should be character(0)

setdiff(unique(gadm_adm1_sel$GID_0), unique(gadm_adm0_sel$GID_0))  # should be character(0)
setdiff(unique(gadm_adm2_sel$GID_0), unique(gadm_adm0_sel$GID_0))  # should be character(0)

sort(unique(gadm_adm0_sel$GID_0))
sort(unique(gadm_adm1_sel$GID_0))
sort(unique(gadm_adm2_sel$GID_0))
