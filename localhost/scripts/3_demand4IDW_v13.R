...
# Regional W and V demand are partitioned into country-origin components so annual
# normalization in MoFuSS cannot transfer demand between components.
...
.preserve_projected_stack_mass <- function(projected_stack, source_files, channel_label) {
  if (terra::nlyr(projected_stack) != length(source_files)) {
    stop(
      ...
      " layers but ", length(source_files), " source rasters were supplied."
    )
  }

  source_totals <- vapply(
    source_files,
    function(path) as.numeric(terra::global(terra::rast(path), "sum", na.rm = TRUE)[1, 1]),
    numeric(1)
  )
  projected_totals <- as.numeric(
    terra::global(projected_stack, "sum", na.rm = TRUE)[, 1]
  )
  invalid <- !is.finite(source_totals) | !is.finite(projected_totals) |
    (projected_totals == 0 & source_totals != 0)
  if (any(invalid)) {
    stop(
      channel_label, " projection mass correction is undefined for layer(s): ",
  ...
  residuals <- corrected_totals - source_totals
  tolerances <- pmax(0.01, abs(source_totals) * 1e-8)
  if (any(!is.finite(residuals) | abs(residuals) > tolerances)) {
    stop(
      channel_label, " projected-stack mass correction failed for layer(s): ",
      paste(which(!is.finite(residuals) | abs(residuals) > tolerances), collapse = ", "),
      "."
    )
  }

  list(
    raster = corrected_stack,
    audit = data.frame(
      layer = seq_along(source_files),
      source_file = basename(source_files),
      source_total_Mg = source_totals,
      projected_total_before_correction_Mg = projected_totals,
      correction_factor = correction_factors,
      corrected_total_Mg = corrected_totals,
      residual_Mg = residuals,
...
w_projection <- .preserve_projected_stack_mass(
  wf_w_stNoAdj, wf_w_list, "Walking demand"
)
wf_w_st <- w_projection$raster
write.csv(
  w_projection$audit,
  "demand_out/projection_mass_audit_w.csv",
  row.names = FALSE,
  quote = FALSE
)
...
v_projection <- .preserve_projected_stack_mass(
  wf_v_stNoAdj, wf_v_list, "Vehicle demand"
)
wf_v_st <- v_projection$raster
write.csv(
  v_projection$audit,
  "demand_out/projection_mass_audit_v.csv",
  row.names = FALSE,
  quote = FALSE
)
...
#
# Both W and V retain one component per demand-origin country through runtime
# allocation. W is domestic-only. Each V origin may access only itself plus its
# directly documented suppliers in the active A, AB, or ABC evidence policy;
# neither reciprocal nor transitive permission is inferred. Components remain
# separate through HC IDW and are normalized after annual biomass eligibility
# in MoFuSS before being summed. This script prepares inputs only and does not
# execute IDW.
...
if (identical(effective_byregion, "Regional") && as.integer(aoi_poly) == 0L) {
  ...
  character_fields <- setdiff(
    ...
    c("ImporterV", "ComponentSize", "AcceptedEdgeCount", "VSrcA", "VSrcB", "VSrcC")
  )
  ...
  normalize_optional_sources <- function(values) {
    values <- as.character(values)
    values[is.na(values)] <- ""
    trimws(values)
  }
  for (source_field in c("VSrcA", "VSrcB", "VSrcC")) {
    country_direction_rules[[source_field]] <- normalize_optional_sources(
      country_direction_rules[[source_field]]
    )
  }
  component_sizes <- suppressWarnings(as.integer(country_direction_rules$ComponentSize))
  accepted_edge_counts <- suppressWarnings(
    as.integer(country_direction_rules$AcceptedEdgeCount)
  )
  if (anyNA(component_sizes) || any(component_sizes < 1L) ||
      anyNA(accepted_edge_counts) || any(accepted_edge_counts < 0L)) {
    stop("ComponentSize and AcceptedEdgeCount must be non-negative integers.")
  }
  country_direction_rules$ComponentSize <- component_sizes
  country_direction_rules$AcceptedEdgeCount <- accepted_edge_counts

  policy_rows <- country_parameters %>%
    dplyr::filter(Var == "v_permission_tier_policy") %>%
    dplyr::pull(ParCHR)
  if (length(policy_rows) > 1L) {
    stop("parameters.csv contains more than one v_permission_tier_policy row.")
  }
  v_permission_tier_policy <- if (length(policy_rows) == 0L) {
    "ABC"
  } else {
    toupper(gsub("[^ABC]", "", trimws(as.character(policy_rows[[1L]]))))
  }
  if (!v_permission_tier_policy %in% c("A", "AB", "ABC")) {
    stop(
      "v_permission_tier_policy must be A, AB, or ABC; received '",
      v_permission_tier_policy, "'."
    )
  }
  active_v_tiers <- strsplit(v_permission_tier_policy, "", fixed = TRUE)[[1L]]
  ...
  permission_set_id <- .single_metadata_value(
    country_direction_rules$PermissionSetID,
    "PermissionSetID"
  )
  ...
  split_sources <- function(text) {
    if (is.na(text) || !nzchar(trimws(text))) {
      return(character())
    }
    sort(unique(trimws(strsplit(text, ";", fixed = TRUE)[[1L]])))
  }
  allowed_v_sources <- function(iso3) {
    rule_row <- country_direction_rules[country_direction_rules$GID_0 == iso3, ]
    tier_sources <- unlist(lapply(active_v_tiers, function(tier) {
      split_sources(rule_row[[paste0("VSrc", tier)]][[1L]])
    }), use.names = FALSE)
    sort(unique(c(iso3, tier_sources)))
  }
  allowed_v_by_country <- stats::setNames(
    lapply(all_iso3, allowed_v_sources),
    all_iso3
  )
  referenced_v_sources <- sort(unique(unlist(allowed_v_by_country, use.names = FALSE)))
  invalid_v_sources <- setdiff(referenced_v_sources, all_iso3)
  if (length(invalid_v_sources) > 0L) {
    stop(
      "Selected V permissions reference countries outside ", region_run_code,
      ": ", paste(invalid_v_sources, collapse = ", ")
    )
  }
  country_direction_rules <- country_direction_rules %>%
    dplyr::arrange(GID_0) %>%
    dplyr::mutate(
      ImporterVConfigured = ImporterV,
      ImporterV = as.integer(vapply(
        GID_0,
        function(iso3) length(setdiff(allowed_v_by_country[[iso3]], iso3)) > 0L,
        logical(1)
      )),
      DemandRule = ifelse(
        ImporterV == 1L,
        paste0("bilateral_sources_tiers_", v_permission_tier_policy),
        "domestic_sources_only"
      ),
      AllowedSourceISO3 = vapply(
        GID_0,
        function(iso3) paste(allowed_v_by_country[[iso3]], collapse = ";"),
        character(1)
      ),
      VPermissionTierPolicy = v_permission_tier_policy
    )
  write.csv(
  ...
  if (!isTRUE(terra::compareGeom(
    ...
    stop("Regional W and V demand grids must have identical geometry.")
  }

  .assign_demand_origins_to_country <- function(demand_db, channel_label) {
    demand_cells <- terra::cellFromXY(
      country_index_raster,
      as.matrix(demand_db[, c("x", "y")])
    )
    if (anyNA(demand_cells)) {
      stop(
        "Some positive ", channel_label,
        "-demand coordinates fall outside the regional raster."
      )
    }
    ...
    if (is.data.frame(extracted_country_index) ||
        is.matrix(extracted_country_index)) {
      if ("country_index" %in% colnames(extracted_country_index)) {
        extracted_country_index <-
          extracted_country_index[, "country_index"]
      } else {
        extracted_country_index <-
          extracted_country_index[, ncol(extracted_country_index)]
      }
    }
    extracted_country_index <- as.integer(extracted_country_index)

    # A bilinearly projected demand surface can retain positive values in a
    # narrow outer fringe. Assign only those sub-pixel cases to the nearest
    # selected country; anything farther than one output-cell diagonal fails.
    ...
    if (length(unassigned_demand_rows) > 0L) {
      if (terra::is.lonlat(wf_v_st[[1]])) {
        stop(
          "Directional ", channel_label,
          "-demand assignment requires a projected raster CRS."
        )
      }
      unassigned_points <- sf::st_as_sf(
        data.frame(
          demand_row = unassigned_demand_rows,
          x = demand_db$x[unassigned_demand_rows],
          y = demand_db$y[unassigned_demand_rows]
        ),
        coords = c("x", "y"),
      ...
      if (any(outside_tolerance)) {
        stop(
          sum(outside_tolerance), " positive ", channel_label,
          "-demand origin(s) remain outside every selected country by more ",
          "than one raster-cell diagonal (", round(one_cell_diagonal, 2),
      ...
      message(
        "Assigned ", length(unassigned_demand_rows), " ", channel_label,
        "-demand origin(s) in the one-cell reprojection fringe to the nearest ",
        "country (maximum distance ",
        round(max(nearest_country_distance), 2), " map units)."
      )
    }

    if (length(extracted_country_index) != nrow(demand_db) ||
        anyNA(extracted_country_index) ||
        any(!extracted_country_index %in%
            country_index_lookup$country_index)) {
      stop(
        "Every positive ", channel_label,
        "-demand origin must be assigned to exactly one country in the ",
        "selected region."
      )
    }
    country_index_lookup$GID_0[
      match(extracted_country_index, country_index_lookup$country_index)
    ]
  }

  wf_w_db4idw$GID_0 <- .assign_demand_origins_to_country(
    wf_w_db4idw,
    "W"
  )
  wf_v_db4idw$GID_0 <- .assign_demand_origins_to_country(
    wf_v_db4idw,
    "V"
  )

  manifest_rows <- list()
  partitioned_w_dbs <- list()
  ...
  add_manifest_row <- function(job_id, channel, demand_iso3,
    ...
    manifest_rows[[length(manifest_rows) + 1L]] <<- data.frame(
      ...
      DirectionRule = rule,
      VPermissionTierPolicy = ifelse(
        toupper(channel) == "V", v_permission_tier_policy, "not_applicable"
      ),
      ...
      OutputRole = paste0(
        "origin_preserving_", toupper(channel), "_pressure_component"
      ),
      CombineOperation = "runtime_normalize_by_origin_then_sum",
      Status = status,
      stringsAsFactors = FALSE
    )
  }

  # Preserve every W origin country as a separate, strictly domestic component.
  # W demand cannot harvest outside its country of origin.
  write_w_job <- function(origin_iso3) {
    job_id <- paste0("W_", origin_iso3, "_ORIGIN")
    job_db <- wf_w_db4idw %>%
      dplyr::filter(GID_0 == origin_iso3)
    if (nrow(job_db) == 0L) {
      add_manifest_row(
        job_id, "w", origin_iso3, NA_integer_, origin_iso3,
        "origin_country_demand_domestic_sources_only", status = "skipped_no_positive_W_demand"
      )
    ...
    bundle_files <- .write_directional_idw_bundle(
      ...
      source_polygons_pcs = adm0_reg_pcs[adm0_reg_pcs$GID_0 == origin_iso3, ]
    )
    w_values <- job_db[, target_colsw, drop = FALSE]
    bundle <- list(
    ...
    partitioned_w_dbs[[length(partitioned_w_dbs) + 1L]] <<- job_db
    add_manifest_row(
      job_id, "w", origin_iso3, NA_integer_, origin_iso3,
      "origin_country_demand_domestic_sources_only", bundle
    )
    invisible(NULL)
  }

  for (origin_iso3 in all_iso3) {
    write_w_job(origin_iso3)
  }

  if (length(partitioned_w_dbs) == 0L) {
    stop("No positive W demand was assigned to an HC origin-country job.")
  }
  partitioned_w <- dplyr::bind_rows(partitioned_w_dbs)
  original_w_totals <- colSums(
    wf_w_db4idw[, target_colsw, drop = FALSE],
    na.rm = TRUE
  )
  partitioned_w_totals <- colSums(
    partitioned_w[, target_colsw, drop = FALSE],
    na.rm = TRUE
  )
  if (nrow(partitioned_w) != nrow(wf_w_db4idw) ||
      !isTRUE(all.equal(
        as.numeric(original_w_totals),
        as.numeric(partitioned_w_totals),
        tolerance = 1e-10,
        check.attributes = FALSE
      ))) {
    stop("Origin-country W jobs do not exactly conserve the prepared regional demand.")
  }
  ...
  for (origin_iso3 in all_iso3) {
    active_sources <- allowed_v_by_country[[origin_iso3]]
    active_importer <- as.integer(length(setdiff(active_sources, origin_iso3)) > 0L)
    write_v_job(
      paste0("V_", origin_iso3, "_ORIGIN"),
      origin_iso3,
      active_importer,
      active_sources,
      if (active_importer == 1L) {
        paste0("origin_country_demand_bilateral_sources_tiers_",
               v_permission_tier_policy)
      } else {
        "origin_country_demand_domestic_sources_only"
      }
    )
  }
  ...
  writeLines(
    c(
      ...
      "",
      "Prepared by 3_demand4IDW_v13.R. IDW was not executed.",
      "Run every manifest row whose RunOnHCCluster value is TRUE.",
      paste0(
        "W is partitioned into one origin-country job per country with positive ",
        "W demand; each W job is restricted to its own national source domain."
      ),
      "Do not sum W components before MoFuSS; each origin is normalized separately after annual biomass eligibility and then summed.",
      paste0(
        "V is partitioned into one origin-country job per country. Active tier ",
        "policy is ", v_permission_tier_policy,
        "; each origin may use only itself plus its directly documented supplier ",
        "countries in those tiers. No symmetry or transitive access is inferred."
      ),
      "Do not sum V components before MoFuSS; each origin is normalized to its own demand after annual biomass eligibility and then summed.",
