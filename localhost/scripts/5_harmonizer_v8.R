...
.harmonize_directional_hc_jobs <- function(
  ...
  required_fields <- c(
    "JobID", "Channel", "DemandISO3", "AllowedSourceISO3", "DemandRows", "DemandTable",
  ...
  if (any(!ready_channels %in% c("W", "V")) ||
    ...
    stop("Directional HC execution requires at least one W job and one V job.")
  }
  ready_w_iso3 <- trimws(as.character(
    manifest$DemandISO3[ready_rows][ready_channels == "W"]
  ))
  if (any(!grepl("^[A-Z]{3}$", ready_w_iso3)) || anyDuplicated(ready_w_iso3)) {
    stop("Directional W jobs require one unique three-letter DemandISO3 each.")
  }
  ...
  expected_operations <- rep(
    "runtime_normalize_by_origin_then_sum",
    length(ready_channels)
  )
  if (any(trimws(as.character(
    manifest$CombineOperation[ready_rows]
  )) != expected_operations)) {
    stop(
      "Every directional W/V job must use component-preserving runtime ",
      "normalization."
    )
  }
