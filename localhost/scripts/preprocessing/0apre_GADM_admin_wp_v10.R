...
# created below.
#
# Version 10 uses M85_B30_V1: the minimum evidence-connected computational
# landscapes needed to contain the 30 accepted directed bilateral charcoal
# permissions. Across 111 demand-ready countries this yields 85 regions: nine
# multi-country connected components and 76 domestic-only singletons. The
# largest region contains 13 countries. Region membership is deliberately
# non-directional; V direction is carried by VSrcA/VSrcB/VSrcC, while W is
# restricted to each demand-origin country by 3_demand4IDW_v13.R.
#
# Rebuild the adapters reproducibly with admin_regions/build_M85_B30_V1.R after
# changing bilateral_charcoal_permissions_v1.csv. Each adapter supplies a
...
regionalization_candidate <- "M85_B30_V1"
bilateral_permission_set <- "B30_V1"
expected_regionalization_countries <- 111L
expected_regionalization_regions <- 85L
expected_bilateral_permissions <- 30L
expected_largest_region <- 13L
...
# M85 adapters carry their stable output suffix in RunCode; no separate map is
# maintained in code.
...
validate_b30_adapter <- function(subregions_df, major_region) {
  required_cols <- c(
  ...
  missing_cols <- setdiff(required_cols, names(subregions_df))
  if (length(missing_cols) > 0) {
    stop(sprintf("[%s] Missing M85/B30 adapter columns: %s",
                 major_region, paste(missing_cols, collapse = ", ")))
  }

  fields_requiring_values <- setdiff(
    required_cols,
    c("ImporterV", "VSrcA", "VSrcB", "VSrcC")
  )
  ...
  if (!identical(unique(trimws(as.character(subregions_df$PermissionSetID))),
                 bilateral_permission_set)) {
    stop(sprintf("[%s] PermissionSetID must be %s.",
                 major_region, bilateral_permission_set))
  }
  if (any(as.integer(subregions_df$ComponentSize) < 1L) ||
      any(as.integer(subregions_df$AcceptedEdgeCount) < 0L)) {
    stop(sprintf("[%s] Invalid component size or accepted edge count.",
                 major_region))
  }
...
validate_b30_adapter(subregionsSSA, "SSA")
validate_b30_adapter(subregionsLATAM, "LATAM")
validate_b30_adapter(subregionsASIA, "ASIA")
validate_b30_adapter(subregionsOCEANIA, "OCEANIA")
validate_b30_adapter(subregionsNorAfr, "NorAfr")
...
permissions <- read_subregions(bilateral_permissions_v, "Bilateral permissions")
required_permission_cols <- c(
  "PermissionSetID", "Direction", "SupplierISO3", "ImporterISO3",
  "EvidenceTier", "ModelDefault"
)
missing_permission_cols <- setdiff(required_permission_cols, names(permissions))
if (length(missing_permission_cols) > 0L) {
  stop("Bilateral permission table is missing columns: ",
       paste(missing_permission_cols, collapse = ", "))
}
permissions <- permissions %>%
  dplyr::filter(
    PermissionSetID == bilateral_permission_set,
    as.logical(ModelDefault)
  )
if (nrow(permissions) != expected_bilateral_permissions ||
    anyDuplicated(permissions$Direction) ||
    any(!permissions$EvidenceTier %in% c("A", "B", "C")) ||
    any(paste0(permissions$SupplierISO3, ">", permissions$ImporterISO3) !=
        permissions$Direction)) {
  stop("The active bilateral permission set is not the audited 30-edge A/B/C set.")
}
permission_regions <- permissions %>%
  dplyr::left_join(
    selected_extent %>% dplyr::select(GID_0, CandidateRegionID),
    by = c("SupplierISO3" = "GID_0")
  ) %>%
  dplyr::rename(SupplierRegionID = CandidateRegionID) %>%
  dplyr::left_join(
    selected_extent %>% dplyr::select(GID_0, CandidateRegionID),
    by = c("ImporterISO3" = "GID_0")
  ) %>%
  dplyr::rename(ImporterRegionID = CandidateRegionID)
if (anyNA(permission_regions$SupplierRegionID) ||
    anyNA(permission_regions$ImporterRegionID) ||
    any(permission_regions$SupplierRegionID != permission_regions$ImporterRegionID)) {
  stop("Every accepted bilateral flow must be internal to one computational region.")
}
if (length(demand_ready_iso) != expected_regionalization_countries ||
    ...
    dplyr::n_distinct(region_definitions$RunCode) !=
      expected_regionalization_regions ||
    max(as.integer(selected_extent$ComponentSize)) != expected_largest_region ||
...
message(sprintf(
  paste0(
    "%s validated: %d countries, %d minimum evidence-connected regions, ",
    "%d directed permissions, largest region %d countries."
  ),
  regionalization_candidate, nrow(selected_extent), nrow(region_definitions),
  nrow(permissions), max(as.integer(selected_extent$ComponentSize))
))
