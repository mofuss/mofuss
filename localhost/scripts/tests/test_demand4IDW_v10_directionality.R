...
test_dir <- tempfile("demand4idw_v10_directionality_")
...
stopifnot(
  ...
  identical(
    manifest$JobID,
    c(
      "W_AAA_ORIGIN", "W_BBB_ORIGIN",
      "V_GLEA_IMPORTERS", "V_BBB_DOMESTIC"
    )
  ),
  ...
  all(
    manifest$AllowedSourceISO3[manifest$Channel == "W"] == "AAA;BBB"
  ),
  sum(manifest$DemandRows[manifest$Channel == "W"]) == 2L,
  sum(manifest$DemandRows[manifest$Channel == "V"]) == 3L
)

w_aaa_demand <- read.csv(
  "to_idw/HC_jobs/W_AAA_ORIGIN/BaU_fwch_w.csv",
  check.names = FALSE
)
w_bbb_demand <- read.csv(
  "to_idw/HC_jobs/W_BBB_ORIGIN/BaU_fwch_w.csv",
  check.names = FALSE
)
stopifnot(
  identical(w_aaa_demand$ID, 31L),
  identical(w_bbb_demand$ID, 32L),
  sum(w_aaa_demand$`2000_fw_w`) + sum(w_bbb_demand$`2000_fw_w`) == 70,
  sum(w_aaa_demand$`2001_fw_w`) + sum(w_bbb_demand$`2001_fw_w`) == 74
)
...
cat("DEMAND4IDW_V10_DIRECTIONALITY_OK\n")
