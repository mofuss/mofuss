...
# MoFuSS ----
# Script: 6e_prepare_directional_IDW_inputs_v2.R
# Version: 2
# Date: Sep 2026
...
# already contains a locality. W origin-country metadata is retained for
# component-preserving installation and runtime normalization.
#
# Inputs:
...
prepare_directional_idw_inputs <- function(
  ...
  w_operations <- trimws(as.character(w_jobs$CombineOperation))
  if (any(w_operations != "runtime_normalize_by_origin_then_sum")) {
    .idw_stop(
      "Every W job must use CombineOperation=",
  ...
  readme_lines <- c(
    ...
    paste0(
      "Keep all ", sum(ready_manifest$Channel == "W"),
      " W origin-country outputs separate; do not sum them on the HC cluster."
    ),
    "MoFuSS v9 normalizes each W component to its origin-country demand after annual biomass eligibility, then sums the components.",
    if (sum(v_rows) > 1L) {
