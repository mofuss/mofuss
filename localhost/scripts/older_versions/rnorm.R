...
if(length(args)==0){
	...
	Histograms.per.Fig_FOR=5
	Histograms.per.Fig_TOF=5
	AGBmap=1
	OSType=64
	BaUvsICS="BaU"
	LUCmap_v = 1
...
if (LUCmap_v == 1) {
  ...
  if (file.exists("LULCC/TempTables/growth_parameters1.csv") == TRUE) {
    ...
    write.csv(LULC_Categories1,"Temp/LULC_Categories.csv",row.names = FALSE)
  }
  ...
  if (file.exists("LULCC/TempTables/growth_parameters2.csv") == TRUE) {
    ...
    write.csv(LULC_Categories2,"Temp/LULC_Categories.csv",row.names = FALSE)
  }
  ...
  if (file.exists("LULCC/TempTables/growth_parameters3.csv") == TRUE) {
    ...
    write.csv(LULC_Categories3,"Temp/LULC_Categories.csv",row.names = FALSE)
  }
...
#rmax values ----
	...
	for(i in min5_FOR:max5_FOR) {
		if (data_FOR[i,3]==0) {
			...
			r1[1]<-r1max
			hist((r1*100),nclass=15,xlab="rmax",ylab="Frequency",main=lulc1_name,sub=expression("% yr"^{-1}*""),col="grey")
			r1<-as.data.frame(r1)
...
#K values ----
	...
	for(i in min5_FOR:max5_FOR) {
		if (data_FOR[i,3]==0) {
			...
			k1[1]<-k1max
			hist(k1,nclass=15,xlab="K",ylab="Frequency",main=lulc2_name,sub=expression("tDM ha"^{-1}*""),col="grey")
			k1<-as.data.frame(k1)
...
# Initial Stock values----
	...
	for(i in min5_FOR:max5_FOR) {
		if (data_FOR[i,3]==0) {
			...
			st1[1]<-inst1
			hist(st1,nclass=15,xlab="Initial Stock",ylab="Frequency",main=lulc3_name,sub=expression("tDM ha"^{-1}*""),col="grey")
...
if (max_TOF!=0) {
	...
	for(i in min5_TOF:max5_TOF) {
		...
		kTOF[1]<-kTOFmax
		hist(kTOF,nclass=15,xlab="Available fuelwood from pruning",ylab="Frequency",main=lulc4_name,sub=expression("tDM ha"^{-1}*" yr"^{-1}*""),col="grey")
...
rmax_all_3=data.frame(Key=c(1:MC),rmax_all_2) 
