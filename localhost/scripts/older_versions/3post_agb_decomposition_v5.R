...
resolve_relocated_manifest_dir <- function(recorded, expected, field, manifest_path) {
  ...
  recorded_path <- resolve_path(recorded, dirname(manifest_path), FALSE, "any")
  expected <- resolve_path(expected, getwd(), TRUE, "dir")
  if (identical(v5_path_key(recorded_path), v5_path_key(expected))) {
  ...
  stopf(
    paste0(
      "Pairing manifest field '%s' points to a different existing or ",
...
parameter_geography <- function(parameters) {
  ...
  geography <- if (identical(scope_type, "country")) {
    parameters$country_iso
  } else if (identical(scope_type, "regional")) {
    parameters$region
  } else {
    stopf(
      "Unsupported byregion value '%s'. Expected 'Country' or 'Regional'.",
      parameters$byregion
    )
  }
  geography <- trimws(as.character(geography))
  if (length(geography) != 1L || is.na(geography) || !nzchar(geography)) {
    stopf("Scenario parameters contain an empty geography for %s.", parameters$byregion)
  }
  geography
}
...
read_pairing_provenance <- function(cfg, bau, ics, pairing_policy) {
  ...
  if (!file.exists(path) || dir.exists(path)) {
    ...
    current_ref <- resolve_relocated_manifest_dir(
      ...
      cfg$ics_dir, "current_scenario_dir", path
    )
    source_ref <- resolve_relocated_manifest_dir(
      manifest_value("bau_source_dir", "bau_source_rel"),
      cfg$bau_dir, "bau_source_dir", path
    )
    ...
    expected_geography <- parameter_geography(ics)
    checks <- c(
      ...
      geography = identical(toupper(value("geography")), toupper(expected_geography)),
