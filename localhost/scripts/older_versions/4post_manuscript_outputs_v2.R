...
# MoFuSS ----
# Script: 4post_manuscript_outputs_v2.R
# Version: 2
# Date: September 2026
...
# and 300-dpi map figures with country boundaries and labels. When the Monte
# Carlo sample meets the configured uncertainty threshold, also writes regional
# and country MC-all tables, mean/SD rasters, and a mean/uncertainty map figure.
...
write_country_table_png <- function(x, path, title, subtitle) {
  if (!identical(names(x)[[1L]], "Country") || ncol(x) < 2L || !nrow(x)) {
    stopf("Country PNG table requires a nonempty Country column followed by metrics.")
  }
  metric_columns <- names(x)[-1L]
  for (column in metric_columns) {
    values <- suppressWarnings(as.numeric(x[[column]]))
    if (any(!is.finite(values))) {
      stopf("Country table column '%s' contains non-finite values.", column)
    }
  }
  ...
  left_margin <- 0.22
  right_margin <- 0.22
  table_width <- country_width + length(metric_columns) * metric_width
  width_in <- left_margin + table_width + right_margin
  title_height <- 0.54
  subtitle_height <- 0.34
  header_height <- 0.78
  body_height <- 0.38
  height_in <- 0.22 + title_height + subtitle_height + 0.08 +
    header_height + nrow(x) * body_height + 0.25

  png_args <- list(
    filename = path, width = width_in, height = height_in,
    units = "in", res = TABLE_PNG_DPI, pointsize = 10, bg = "white"
  )
  if (isTRUE(capabilities("cairo"))) png_args$type <- "cairo"
  do.call(grDevices::png, png_args)
  on.exit(grDevices::dev.off(), add = TRUE)
  ...
  graphics::par(mar = c(0, 0, 0, 0), xaxs = "i", yaxs = "i", family = "sans")
  graphics::plot.new()
  graphics::plot.window(xlim = c(0, width_in), ylim = c(0, height_in), xaxs = "i", yaxs = "i")
  left <- left_margin
  right <- width_in - right_margin
  edges <- c(left, left + country_width + seq.int(0L, length(metric_columns)) * metric_width)
  y <- height_in - 0.20
  graphics::text(left, y, title, adj = c(0, 1), font = 2, cex = 1.12, col = "#172B4D")
  y <- y - title_height
  graphics::text(left, y, subtitle, adj = c(0, 1), cex = 0.78, col = "#44546A")
  y <- y - subtitle_height - 0.05

  header_bottom <- y - header_height
  graphics::rect(left, header_bottom, right, y, col = "#234E70", border = NA)
  graphics::text(left + 0.10, mean(c(y, header_bottom)), "Country",
                 adj = c(0, 0.5), font = 2, cex = 0.78, col = "white")
  for (j in seq_along(metric_columns)) {
    label <- paste(strwrap(metric_columns[[j]], width = 16L), collapse = "\n")
    graphics::text(mean(edges[c(j + 1L, j + 2L)]), mean(c(y, header_bottom)), label,
                   font = 2, cex = 0.66, col = "white")
  }
  y <- header_bottom

  for (i in seq_len(nrow(x))) {
    row_bottom <- y - body_height
    fill <- if (i %% 2L == 0L) "#F4F6F8" else "white"
    graphics::rect(left, row_bottom, right, y, col = fill, border = NA)
    graphics::segments(left, row_bottom, right, row_bottom, col = "#D5DCE3", lwd = 0.55)
    graphics::text(left + 0.10, mean(c(y, row_bottom)), as.character(x$Country[[i]]),
                   adj = c(0, 0.5), cex = 0.74, col = "#1F2933")
    for (j in seq_along(metric_columns)) {
      value <- as.numeric(x[[metric_columns[[j]]]][[i]])
      graphics::text(
        edges[[j + 2L]] - 0.08, mean(c(y, row_bottom)),
        formatC(value, format = "f", digits = 3L, big.mark = ","),
        adj = c(1, 0.5), cex = 0.70, col = "#1F2933"
      )
    }
    y <- row_bottom
  }
  graphics::rect(left, y, right, y + nrow(x) * body_height,
                 border = "#9AA5B1", lwd = 0.8)
  for (edge in edges[-c(1L, length(edges))]) {
    graphics::segments(edge, y, edge, y + nrow(x) * body_height + header_height,
                       col = "#C7CED6", lwd = 0.55)
  }
  invisible(path)
}
...
country_compact_png_paths <- sub("\\.csv$", ".png", country_compact_csv_paths)
...
for (configuration in CONFIGURATION_ORDER) {
  ...
  write_country_table_png(
    country_compact_tables[[configuration]],
    country_compact_png_paths[[configuration]],
    sprintf(
      "%s country results: %s, %s",
      region_name, tools::toTitleCase(configuration), period_tag
    ),
    paste0(
      "First Monte Carlo realization (MC1). All columns are MtCO2e per period, ",
      "except Annual (MtCO2e yr^-1). Country values use spatial incidence."
    )
  )
}
...
expected_files <- c(
  ...
  file.path("tables", basename(country_compact_png_paths)),
