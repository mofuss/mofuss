...
.v13_parameter_geography <- function(parameters) {
  ...
  geography <- if (identical(scope_type, "country")) {
    ...
    .v9_stop(
      "Unsupported byregion value '", parameters$byregion,
  ...
  if (length(geography) != 1L || is.na(geography) || !nzchar(geography)) {
    .v9_stop("Scenario parameters contain an empty geography for ", parameters$byregion, ".")
  }
  geography
}
...
.v13_internal_pairs <- function(scenario_dirs = SCENARIO_DIRS) {
  ...
  pairs <- lapply(alternatives, function(i) {
    ...
    scope_name <- .v13_parameter_geography(a)
...
.v13_read_bypass_provenance <- function(bau_root, ics_root, bau_par, ics_par) {
  ...
  expected_geography <- .v13_parameter_geography(ics_par)
  exact_checks <- c(
