# MoFuSS atlas with 2 zooms from KML polygons + hillshade on zooms
# Requires: tmap (plot mode), terra, sf, classInt, fs, dplyr, stringr, readr
...
two_map_vertical_gadm <- function(
  ...
  if (save && make_zooms && !is.null(bboxes_sf) && nrow(bboxes_sf) > 0) {
    ...
    # hillshade aligned to full raster grid, then we crop to each zoom
    hs_full <- NULL
    ...
    make_zoom_combo <- function(zoom_id) {
      ...
      # crop admin layers so they DON'T force extent back to the full country
      adm0_z <- sf::st_crop(adm0_sf, bb_bbox)
      adm1_z <- sf::st_crop(adm1_sf, bb_bbox)
      
      # crop hillshade (if exists)
      hs_z <- NULL
      if (!is.null(hs_full)) {
      ...
      make_zoom_map <- function(r, subtitle) {
        ...
        # Background for zoom: hillshade if available, otherwise plain grey polygon
        if (!is.null(hs_z)) {
          tm <- tm +
            tm_shape(hs_z) +
            tm_raster(
        ...
        # Main raster (NA fully transparent for zooms!)
        tm <- tm +
          tm_shape(r) +
          tm_raster(style="fixed", breaks=brks, labels=labels, palette="viridis",
        ...
        # Force extent to bbox so the zoom fills the whole figure
        tm +
          tm_compass(position=c("left","top"), size=0.7) +
          tm_scalebar(position=c("left","bottom"), text.size=0.5) +
          tm_layout(main.title = as.character(paste0(subtitle,"  •  ",zoom_label)), main.title.size = 1.2,
                    legend.outside=FALSE, legend.position=c("right","top"),
                    legend.bg.color="white", legend.bg.alpha=0.95,
                    frame=TRUE, inner.margins=c(0.01,0.02,0.02,0.02)) +
          tm_shape(adm0_z) # harmless anchor for bbox-based extent
      }
      
      # Critical: tmap derives extent from shapes; ensure it uses the CROPPED ones
