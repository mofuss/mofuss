...
#---------------------------------------------------------------------#
# 2. Define anchor shares for Biomass & Charcoal (2009, 2016) ----
#---------------------------------------------------------------------#
...
# 
# Replace Biomass & Charcoal with interpolated share_target.
# 
# If Biomass+Charcoal total > 1 → scale them down so they sum to 1, others → 0.
# 
# Else:
#   
#   “Other fuels” keep relative proportions, but are scaled so that
# sum(other_new_shares) = 1 – (Biomass+Charcoal).
...
pop_slv_ru_adjusted <- pop_slv_ru_shares %>%
  ...
  group_by(iso3, country, region, area, year) %>%
  mutate(
    is_BC = fuel %in% c("Biomass", "Charcoal"),
...
pop_slv_ru_adjusted %>%
  filter(area == "Urban", fuel == "Biomass", year >= 2009) %>%
