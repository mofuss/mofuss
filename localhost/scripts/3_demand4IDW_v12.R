...
# Regional W and V demand are partitioned into directional components so annual
...
#
# Both W and V retain their demand-side components through runtime allocation.
# Every W origin country may reach the full regional source domain through the
# border-friction surface. V importer demand is pooled and may also reach the
# full region; each non-importer V component remains domestic-only. Components
# must remain separate through HC IDW and are normalized after annual biomass
# eligibility in MoFuSS before being summed. This script prepares inputs only;
# it does not execute IDW.
...
if (identical(effective_byregion, "Regional") && as.integer(aoi_poly) == 0L) {
  ...
  # Preserve every W origin country as a separate component. Each component may
  # use the full regional source domain, but it is normalized to its own annual
  # demand inside MoFuSS after the current biomass-eligibility mask. This stops
  # biomass scarcity in one country from reassigning its demand to another
  # country's component while retaining friction-limited cross-border walking.
  write_w_job <- function(origin_iso3) {
    ...
    if (nrow(job_db) == 0L) {
      add_manifest_row(
        job_id, "w", origin_iso3, NA_integer_, all_iso3,
        "origin_country_demand_regional_sources", status = "skipped_no_positive_W_demand"
      )
    ...
    bundle_files <- .write_directional_idw_bundle(
      ...
      source_polygons_pcs = adm0_reg_pcs
    )
    ...
    add_manifest_row(
      job_id, "w", origin_iso3, NA_integer_, all_iso3,
      "origin_country_demand_regional_sources", bundle
    )
    invisible(NULL)
  }
  ...
  writeLines(
    c(
      ...
      "",
      "Prepared by 3_demand4IDW_v12.R. IDW was not executed.",
