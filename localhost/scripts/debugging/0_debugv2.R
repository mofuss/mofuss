...
library(raster)
# Demand from 1a_wf_demand_script, to cross-check with MoFuSS datasets AFTER projection
...
(HSRL_bio_demand_2050 <- raster("D:/Demand/demand_out/HSRL_bio_demand_2050.tif") %>%
    cellStats(stat='sum', na.rm=TRUE))
(HSRL_cha_demand_2010 <- raster("D:/Demand/demand_out/HSRL_cha_demand_2010.tif") %>%
    cellStats(stat='sum', na.rm=TRUE))
(HSRL_cha_demand_2050 <- raster("D:/Demand/demand_out/HSRL_cha_demand_2050.tif") %>%
    cellStats(stat='sum', na.rm=TRUE))
...
(HSRL_wftons_w_2010 <- raster("D:/Demand/demand_out/HSRL_wftons_w_2010.tif") %>%
  cellStats(stat='sum', na.rm=TRUE))
HSRL_wftons_v_2010+HSRL_wftons_w_2010

(HSRL_wftons_v_2050 <- raster("D:/Demand/demand_out/HSRL_wftons_v_2050.tif") %>%
  cellStats(stat='sum', na.rm=TRUE))
(HSRL_wftons_w_2050 <- raster("D:/Demand/demand_out/HSRL_wftons_w_2050.tif") %>%
  cellStats(stat='sum', na.rm=TRUE))
HSRL_wftons_v_2050+HSRL_wftons_w_2050

# World Mercator
(wf_w_st_2010_db <- raster("D:/Demand/demand_out/wf_w_st_2010_db.tif") %>%
  cellStats(stat='sum', na.rm=TRUE))
(wf_v_st_2010_db <- raster("D:/Demand/demand_out/wf_v_st_2010_db.tif") %>%
  cellStats(stat='sum', na.rm=TRUE))
wf_w_st_2010_db+wf_v_st_2010_db

(wf_w_st_2050_db <- raster("D:/Demand/demand_out/wf_w_st_2050_db.tif") %>%
    cellStats(stat='sum', na.rm=TRUE))
(wf_v_st_2050_db <- raster("D:/Demand/demand_out/wf_v_st_2050_db.tif") %>%
    cellStats(stat='sum', na.rm=TRUE))
wf_w_st_2050_db+wf_v_st_2050_db

proj_factor_2010 <- (HSRL_wftons_v_2010+HSRL_wftons_w_2010)/wf_w_st_2010_db+wf_v_st_2010_db
proj_factor_2050 <- HSRL_wftons_v_2050+HSRL_wftons_w_2050/wf_w_st_2050_db+wf_v_st_2050_db
proj_factor <- mean(projection_factor_1,projection_factor_2)

# World Mercator
(wf_w_st_2010_db_pf <- (raster("D:/Demand/demand_out/wf_w_st_2010_db.tif") * proj_factor) %>%
     cellStats(stat='sum', na.rm=TRUE))
(wf_v_st_2010_db_pf <- (raster("D:/Demand/demand_out/wf_v_st_2010_db.tif") * proj_factor) %>%
    cellStats(stat='sum', na.rm=TRUE))
wf_w_st_2010_db_pf+wf_v_st_2010_db_pf

(wf_w_st_2050_db_pf <- (raster("D:/Demand/demand_out/wf_w_st_2050_db.tif") * proj_factor) %>%
    cellStats(stat='sum', na.rm=TRUE))
(wf_v_st_2050_db_pf <- (raster("D:/Demand/demand_out/wf_v_st_2050_db.tif") * proj_factor) %>%
    cellStats(stat='sum', na.rm=TRUE))
wf_w_st_2050_db_pf+wf_v_st_2050_db_pf  
  


# Before and after projection for 2010
...
cellStats(HSRL_wftons_v_2010, stat='sum', na.rm=TRUE)
