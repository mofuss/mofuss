...
# MoFuSS ----
# Script: 6f_install_directional_IDW_outputs_v2.R
# Version: 2
# Date: Sep 2026
...
# consumed by the Dinamica EGO model. W origin-country components remain
# separate for annual, post-eligibility normalization in MoFuSS v9; a summed W
# raster is also installed only for Patcher ranking and diagnostics. Every V
# component is summed pixel by pixel exactly once for each period.
...
install_directional_idw_outputs <- function(
  ...
  w_demand_by_job <- lapply(seq_along(w_jobs), function(component_index) {
    manifest_row <- w_rows[[component_index]]
    totals <- .idw6f_read_annual_demand(
      as.character(manifest$DemandTable[[manifest_row]]),
      "W",
      paste0("W origin component ", w_jobs[[component_index]])
    )
    if (!identical(as.integer(names(totals)), annual_years)) {
      .idw6f_stop(
        "W origin component ", w_jobs[[component_index]],
        " does not cover the manifest's complete annual period."
      )
    }
    totals
  })
  w_demand_matrix <- do.call(rbind, w_demand_by_job)
  rownames(w_demand_matrix) <- w_jobs
  colnames(w_demand_matrix) <- annual_years

  # The component tables must reconstruct the exact regional W trajectory that
  # the model's existing demand branch will report and use for diagnostics.
  for (year_index in seq_along(annual_years)) {
    regional_lookup_path <- file.path(
      in_root,
      "DemandScenarios",
      sprintf("fwuse_W_ext_fwdef%02d.csv", annual_periods[[year_index]])
    )
    if (!file.exists(regional_lookup_path)) {
      .idw6f_stop(
        "Regional W demand lookup is missing: ", regional_lookup_path
      )
    }
    regional_total <- .idw6f_read_lookup_total(
      regional_lookup_path,
      paste0("Regional W demand for ", annual_years[[year_index]])
    )
    component_total <- sum(w_demand_matrix[, year_index])
    if (!.idw6f_demand_equal(regional_total, component_total)) {
      .idw6f_stop(
        "W origin components do not reconstruct regional demand for ",
  ...
  w_component_index <- data.frame(
    ComponentIndex = seq_along(w_jobs),
    DemandISO3 = w_iso3,
    JobID = w_jobs,
    DirectionRule = as.character(manifest$DirectionRule[w_rows]),
    AllowedSourceISO3 = as.character(manifest$AllowedSourceISO3[w_rows]),
    FirstYearDemandTons = as.numeric(w_demand_matrix[, 1L]),
    LastYearDemandTons = as.numeric(w_demand_matrix[, ncol(w_demand_matrix)]),
    stringsAsFactors = FALSE,
    check.names = FALSE
  )
  ...
  component_index_path <- file.path(
    in_root, "DemandScenarios", "W_origin_component_index.csv"
  )
  demand_audit_path <- file.path(
  ...
  for (period_index in seq_along(periods)) {
    ...
    if (w_stats$positive == 0L || w_stats$minimum < 0 ||
        !is.finite(w_stats$maximum)) {
      .idw6f_stop("Combined W ranking output is invalid for period ", suffix, ".")
    }
  ...
  for (year_index in seq_along(annual_years)) {
    demand_stage <- file.path(
      staging_root,
      sprintf("W_origin_demand%02d.csv", annual_periods[[year_index]])
    )
    demand_lookup <- data.frame(
      Key = seq_along(w_jobs),
      Value = as.numeric(w_demand_matrix[, year_index]),
      check.names = FALSE
    )
    write.csv(
      demand_lookup,
  ...
  demand_audit <- do.call(rbind, demand_audit_rows)

  staged_component_index <- file.path(staging_root, basename(component_index_path))
  staged_demand_audit <- file.path(staging_root, basename(demand_audit_path))
  ...
  readme_lines <- c(
    ...
    "The top-level W raster is a summed ranking surface only; MoFuSS v9 must not normalize it as one regional demand pool.",
    ...
    "SHA-256 checksums and source paths are recorded in HC_IDW_install_manifest.csv and W_origin_demand_manifest.csv.",
    "CostDistance_IDW runtime parameters (-t and -e) are not embedded in GeoTIFFs and must be preserved with the HPC logs.",
    "Use 7_dyn_Sc17_webmofuss_ctrees_g_v9.egoml with these installed W components."
  )
  ...
  destinations <- c(
    ...
    .idw6f_normalize(component_index_path, must_work = FALSE),
    .idw6f_normalize(demand_audit_path, must_work = FALSE),
