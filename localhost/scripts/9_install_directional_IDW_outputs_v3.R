...
# MoFuSS ----
# Script: 9_install_directional_IDW_outputs_v3.R
# Version: 3
# Date: Sep 2026
...
install_directional_idw_outputs <- function(
  ...
  read_component_demand <- function(rows, jobs, channel) {
    demand_by_job <- lapply(seq_along(jobs), function(component_index) {
      manifest_row <- rows[[component_index]]
      totals <- .idw6f_read_annual_demand(
        as.character(manifest$DemandTable[[manifest_row]]),
        channel,
        paste0(channel, " origin component ", jobs[[component_index]])
      )
      if (!identical(as.integer(names(totals)), annual_years)) {
        .idw6f_stop(
          channel, " origin component ", jobs[[component_index]],
  ...
  w_demand_matrix <- read_component_demand(w_rows, w_jobs, "W")
  v_demand_matrix <- read_component_demand(v_rows, v_jobs, "V")
