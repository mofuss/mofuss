...
test_fun_fnrb_cals <- function(df,country) {
    # Check that when ADM_0 is Tanzania, the abs(fnrb - calculated_fnrb) is less than 0.5
    df_test <- df %>%
        filter(ADM_0 == country) %>%
        mutate(diff = abs(fnrb - 100*calculated_fnrb)) %>%
        filter(diff > 0.5)
    if (nrow(df_test) > 0) {
        print(paste("🚨🚨🚨 Test FNRB FAILED for", country, ": There are", nrow(df_test), "rows where the difference between fnrb and calculated_fnrb is greater than 0.5"))
        print(df_test)
    } else {
