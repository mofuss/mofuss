...
# Clean temps - keep inelegant list of unlinks as its the easier layout for the moment ----
...
unlink("LULCC//TempTables//Friction_lakes_reclass.csv",force=TRUE)
unlink("LULCC//TempTables//Friction_lakes_reclass_calcdist.csv",force=TRUE)
unlink("LULCC//TempTables//Friction_borders_reclass.csv",force=TRUE)
unlink("LULCC//TempTables//Friction_borders_reclass_calcdist.csv",force=TRUE)
...
if (GEpoly == 1) {
  ...
  if (length(list.files("LULCC/SourceData/InVector", ".gpkg")) == 0) { ### CHANGE TO gpkg!! -> if no gpkg asks for shp as no one
    # shell.exec("https://gitlab.com/mofuss/mofuss/-/blob/master/windows/scripts/LULCC/Wizard_imgs/NoKMLs.pdf")
  } else {
    # if (length(list.files("LULCC/SourceData/InVector", ".gpkg")) > 2) { ### CHANGE TO gpkg!! -> if no gpkg asks for shp as no one
    # shell.exec("https://gitlab.com/mofuss/mofuss/-/blob/master/windows/scripts/LULCC/Wizard_imgs/ManySHPs.pdf")
    # unlink("LULCC//SourceData//InVector//*.shp.xml",force=TRUE)
    nrbpolyshp<-choose.files(default=paste0(getwd(),"/LULCC/SourceData/InVector/*"))
    ...
    if (add_subadmin == "YES") { 
      nrbpolyshp1<-choose.files(default=paste0(getwd(),"/LULCC/SourceData/InVector/*"))
      if (!identical(nrbpolyshp1, character(0)) ){
        userarea1 <- st_read(nrbpolyshp1) 
        userarea_GCS1 <- st_transform(userarea1, epsg_gcs)
      }
      
      nrbpolyshp2<-choose.files(default=paste0(getwd(),"/LULCC/SourceData/InVector/*"))
      if (!identical(nrbpolyshp2, character(0)) ){
        userarea2 <- st_read(nrbpolyshp2) 
  ...
  if (!identical(nrbpolyshp1, character(0)) ){
    mask1 <- st_read("LULCC/SourceData/InVector/extent_mask1.gpkg")
    analysisshp1 <- st_intersection(mask1, userarea1)
    analysisshp_GCS1 <- st_transform(analysisshp1, epsg_gcs)
  }
  
  if (!identical(nrbpolyshp2, character(0)) ){
    mask2 <- st_read("LULCC/SourceData/InVector/extent_mask2.gpkg")
...
if (GEpoly != 1) {
  ...
  if (!identical(nrbpolyshp1, character(0)) ){
    ...
    #userarea_ras1 <- raster(userarea1, res=resolution)
    ...
    if (!identical(nrbpolyshp2, character(0)) ){
      ...
      # userarea_ras2 <- raster(userarea2, res=resolution)
