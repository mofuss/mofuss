...
build_mofuss_demand <- function(dem, popWHO) {
  ...
  dem_std <- dem %>%
    transmute(
      ...
      year      = as.integer(year),
      fuel0     = tolower(as.character(fuel)),
      fuel_tons3 = as.numeric(fuel_tons3)
    ) %>%
    mutate(
      fuel = case_when(
        fuel0 == "biomass"     ~ "fuelwood",
    ...
    summarise(
      fuel_tons3 = sum(fuel_tons3, na.rm = TRUE),
      .groups = "drop"
    )
  ...
  mofuss <- who_full %>%
    left_join(
      dem_std %>% dplyr::select(iso3, area, year, fuel, fuel_tons3),
      by = c("iso3", "area", "year", "fuel"),
      relationship = "one-to-one"
    ) %>%
    mutate(
      fuel_tons3 = ifelse(is.na(fuel_tons3), 0, fuel_tons3)
    ) %>%
    dplyr::select(
  ...
  overall_rows <- mofuss %>%
    ...
    dplyr::select(iso3, country, area, fuel, year, people, fuel_tons3)
  ...
  mofuss_or <- mofuss_o %>%
    ...
    dplyr::select(
      ...
      people, pc_fuel, fuel_tons3, fuel_tons_calc
    )
...
anchors_user <- tibble(
  ...
  share_user = c(
    # 2000
    0.70, 0.20, 0.00, 0.00, 0.02, 0.05, 0.01, 0.00, 0.00, 0.00, 0.02,
    
    # 2030
    0.45, 0.12, 0.05, 0.10, 0.12, 0.07, 0.06, 0.01, 0.01, 0.01, 0.00,
    
    # 2050
    0.00, 0.00, 0.10, 0.50, 0.20, 0.05, 0.11, 0.01, 0.01, 0.02, 0.00
  )
)

# If reading from CSV instead:
# anchors_user <- read_csv("your_anchor_table.csv", show_col_types = FALSE)
