...
if (length(MC) != 1L || is.na(MC) || MC < 1L) stop("MC must be one positive integer.")
if (length(IT) != 1L || is.na(IT)) stop("IT must be one integer model-start year.")
if (length(IL) != 1L || !is.finite(IL) || IL <= 0) stop("IL must be positive.")
...
if (!file.exists("LULCC/TempTables/Country.csv")) {
  stop("Run NRB_graphs_datasets_v8.R from a MoFuSS scenario root.")
}
...
if (IT != configured_start) stop("Dinamica IT does not match parameters.csv start_year.")
if (MC != configured_mc) stop("Dinamica MC does not match parameters.csv monte_carlo_runs.")
...
expected_timeseries <- c(agb_CadF, nrb_CadF, con_CadF, connrb_CadF)
missing_timeseries <- expected_timeseries[!file.exists(expected_timeseries)]
if (length(missing_timeseries)) {
  stop(
    "Missing ", length(missing_timeseries),
    " expected MC time-series table(s). First missing file: ",
