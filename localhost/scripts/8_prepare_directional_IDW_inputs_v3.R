...
# MoFuSS ----
# Script: 8_prepare_directional_IDW_inputs_v3.R
# Version: 3
# Date: Sep 2026
# Execution: Source from RStudio after scripts 5, 6 and 7 have completed.
#
...
# already contains a locality. W and V origin-component metadata are retained
# for component-preserving installation and runtime normalization.
#
# Inputs:
...
.idw_prepare_job <- function(
  ...
  data.frame(
    ...
    DemandISO3 = as.character(manifest_row$DemandISO3[[1L]]),
    AllowedSourceISO3 = as.character(manifest_row$AllowedSourceISO3[[1L]]),
    DirectionRule = as.character(manifest_row$DirectionRule[[1L]]),
    Status = "IDW_READY",
    PeriodStart = 1L,
...
prepare_directional_idw_inputs <- function(
  ...
  required_columns <- c(
    ...
    "DemandISO3", "AllowedSourceISO3", "DirectionRule",
  ...
  job_channels <- toupper(trimws(as.character(jobs$Channel)))
  w_jobs <- jobs[job_channels == "W", , drop = FALSE]
  if (nrow(w_jobs) < 1L) {
    .idw_stop("The harmonized HC manifest contains no W origin-country jobs.")
  }
  w_iso3 <- trimws(as.character(w_jobs$DemandISO3))
  if (any(!grepl("^[A-Z]{3}$", w_iso3)) || anyDuplicated(w_iso3)) {
    .idw_stop(
      "Every W job must contain one unique three-letter DemandISO3 value."
    )
  }
  component_operations <- trimws(as.character(jobs$CombineOperation))
  if (any(component_operations != "runtime_normalize_by_origin_then_sum")) {
    .idw_stop(
      "Every W/V job must use CombineOperation=",
  ...
  readme_lines <- c(
    ...
    paste0(
      "Keep all ", nrow(ready_manifest),
      " W/V origin-component outputs separate; do not sum them on the HC cluster."
    ),
    "MoFuSS v11 normalizes each W and V component to its own demand after annual biomass eligibility, then sums the components.",
