...
# MoFuSS ----
# Script: 1_mechanics_verifications_v3.R
# Version: 3
# Date: September 2026
...
#
# Usage:
#   Rscript 1_mechanics_verifications_v3.R [working_dir ...]
#
...
for (arg in args) {
  if (arg %in% c("--help", "-h")) {
    cat(paste0(
      "Usage: Rscript 1_mechanics_verifications_v3.R [working_dir ...] [options]\n",
...
# A regional run legitimately contains one raw raster per constituent country.
# Sum each source independently because country rasters can have different
# extents and therefore cannot safely be combined into one SpatRaster first.
raw_demand_sources <- rbindlist(lapply(years, function(year) {
  rbindlist(lapply(c("w", "v"), function(channel) {
    paths <- if (dir.exists(raw_demand_dir)) {
      list.files(
        raw_demand_dir,
    ...
    if (!length(paths)) {
      return(data.table(
        year = year, channel = channel, source_file = NA_character_,
        source_total_Mg = NA_real_
      ))
    }
    data.table(
      year = year,
      ...
      source_total_Mg = vapply(
        paths,
        function(path) as.numeric(global(rast(path), "sum", na.rm = TRUE)[1, 1]),
        numeric(1)
      )
    )
  }), fill = TRUE)
}), fill = TRUE)
fwrite(raw_demand_sources, file.path(out_dir, "raw_demand_sources.csv"))

raw_demand_sum <- function(year, channel) {
  query_year <- as.integer(year)
  query_channel <- tolower(as.character(channel))
  x <- raw_demand_sources[
    year == query_year & channel == query_channel
  ]
  if (!nrow(x) || any(!is.finite(x$source_total_Mg))) return(NA_real_)
  sum(x$source_total_Mg)
}
raw_demand_source_count <- function(year, channel) {
  query_year <- as.integer(year)
  query_channel <- tolower(as.character(channel))
  x <- raw_demand_sources[
    year == query_year & channel == query_channel &
...
demand_by_year <- rbindlist(lapply(seq_along(years), function(j) {
  ...
  data.table(
    ...
    raw_demand_W_source_files = raw_demand_source_count(years[[j]], "w"),
    raw_demand_V_source_files = raw_demand_source_count(years[[j]], "v"),
...
raw_demand_total_Mg <- sum_complete(demand_by_year$raw_demand_total_Mg)
assigned_demand_total_Mg <- sum_complete(demand_by_year$assigned_expected_total_Mg)
raw_minus_assigned_Mg <- raw_demand_total_Mg - assigned_demand_total_Mg
raw_minus_assigned_percent <- pct(raw_minus_assigned_Mg, raw_demand_total_Mg)
max_annual_raw_assigned_gap_Mg <- max_safe(abs(demand_by_year$preassignment_gap_Mg))
raw_assigned_reconciled <- demand_complete && is.finite(raw_minus_assigned_Mg) &&
  abs(raw_minus_assigned_Mg) <= max(1, abs(raw_demand_total_Mg) * 1e-9)
scaling_audit <- data.table(
  ...
  status = c(
    ...
    if (!demand_complete) {
      "NOT AUDITED"
    } else if (raw_assigned_reconciled && core_unit_reconciled) {
      "RECONCILED"
    } else {
      "COMPLETE / GAP REPORTED"
    },
    if (!is.finite(area_bias_percent)) "NOT AUDITED" else "PHYSICAL-AREA DIFFERENCE",
    if (is_mercator) "MODEL CONVENTION / REPORTING CAUTION" else "CHECK CRS CONVENTION",
  ...
  evidence = c(
    ...
    sprintf("Raw-demand years complete=%s; assigned W/V demand years complete=%s; raw minus assigned=%.3f Mg (%.6f%% of raw); maximum annual absolute gap=%.3f Mg; max expected-map versus demand-table difference=%s Mg", raw_demand_complete, assigned_demand_complete, raw_minus_assigned_Mg, raw_minus_assigned_percent, max_annual_raw_assigned_gap_Mg, ifelse(is.finite(expected_map_demand_error), sprintf("%.6f", expected_map_demand_error), "not available")),
    sprintf("This is not a cell-area calculation error. A projected 100-ha cell has geodesic ground area %.3f-%.3f ha (mean %.3f ha) over this AOI; nominal minus geodesic mean=%.3f%%.", geodesic_min_ha, geodesic_max_ha, geodesic_mean_ha, area_bias_percent),
    "The completed run uses 100 nominal projected hectares for table-derived K/TOF scaling, while harmonized AGB and CTrees A preserve geodesic mass. EPSG:3395 is not equal-area. Keep native mass accounting explicit and use geodesic cell area for physical densities/totals.",
...
auxiliary_provenance <- function(filename, inspect_engine_reference = FALSE) {
  ...
  status <- if (!exists) {
    ...
    paste0("LEGACY ROOT ARTIFACT: ", paste(flags, collapse = " + "), " / NOT USED")
  } else "LEGACY ROOT ARTIFACT / NOT USED"
...
raw_w_source_range <- range(demand_by_year$raw_demand_W_source_files, na.rm = TRUE)
raw_v_source_range <- range(demand_by_year$raw_demand_V_source_files, na.rm = TRUE)
...
if (!length(c(missing_raw_years, missing_assigned_years))) {
  completeness_lines <- c(
    ...
    sprintf(
      paste0(
        "  Raw-demand inventory and assigned W/V demand tables are complete for every reporting year; ",
        "the audit independently summed %.0f-%.0f W and %.0f-%.0f V source rasters per year."
      ),
      raw_w_source_range[[1L]], raw_w_source_range[[2L]],
      raw_v_source_range[[1L]], raw_v_source_range[[2L]]
    )
  )
}
...
area_conclusion_line <- sprintf(
  paste0(
    "  The cell-area calculation is correct: each projected grid cell is nominally %.3f ha, ",
    "whereas its geodesic ground area is %.3f-%.3f ha (mean %.3f ha) in this AOI; ",
    "nominal exceeds the geodesic mean by %s. %s"
  ),
  nominal_area_ha, geodesic_min_ha, geodesic_max_ha, geodesic_mean_ha,
  fmt_percent(area_bias_percent, 3L),
  if (is_mercator) {
    paste0(
      "World Mercator is not equal-area, so this is a model-unit convention and physical-density reporting caution, ",
      "not evidence that terra::cellSize is wrong."
    )
  } else {
